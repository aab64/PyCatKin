...
for Ti, temperature in enumerate([500, 650, 800]):
    for study in ['dry', 'wet']:
        ...
        bsOs = np.linspace(start=-5, stop=0, num=10, endpoint=True)
        bsOs = [-4, -3, -2, -1.5, -1, -0.5]
        ...
        # for bind, metal in enumerate(all_metals):
            ...
            #                  csv_path=results_dir + 'sr/%s/%1.0f/' % (metal, temperature))
            #
            # sys.reaction_terms(y=sys.full_steady)
            # i5 = list(sys.reactions.keys()).index('r5_rdr')
            # i9 = list(sys.reactions.keys()).index('r9_rdr')
            # tofs[bind] = (sys.rates[i5, 0] - sys.rates[i5, 1]) + (sys.rates[i9, 0] - sys.rates[i9, 1])
...
for study in ['dry', 'wet']:
    ...
    for ti, temperature in enumerate([500, 650, 800]):
        for mi, metal in enumerate([-4, -3, -2, -1.5, -1]):
            ...
            for xi, x in enumerate(np.linspace(start=mi, stop=mi + 1 - 0.5 / (len(rnames) + 1), num=len(rnames))):
                ax[ti].bar(x + 0.5 / (len(rnames) + 1), drcs[xi], width=1 / (len(rnames) + 1), color=cmap(xi),
                           label=rnames[xi].split('_')[0] if mi == 0 else '')
            ax[ti].plot((mi + 1, mi + 1), (-1, 1), ':', color='lightgrey')
            ax[ti].plot((0, 5), (1, 1), ':', color='lightgrey')
        ax[ti].set(title='%1.0f K' % temperature, xlabel='*O energy [eV]', ylim=(0, 1.5), xlim=(0, 5),
                   xticks=(0.5, 1.5, 2.5, 3.5, 4.5), xticklabels=(-4, -3, -2, -1.5, -1))
    ax[0].set(ylabel='DRC')
